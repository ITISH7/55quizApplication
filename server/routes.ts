...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/answers", requireAuth, async (req: any, res) => {
    try {
      ...
      try {
        if (isCorrect) {
          console.log('✅ Answer is CORRECT - calculating points...');
          ...
          if (question.isBonus) {
            ...
            console.log('⚡ Regular question - checking scoring type...');
            ...
            console.log('⚙️ Raw speedScoringConfig:', quiz?.speedScoringConfig);
            
            if (quiz?.scoringType === "speed" && quiz.speedScoringConfig) {
              console.log('🚀 SPEED SCORING with custom config!');
              ...
              if (speedConfig.length > 0) {
                ...
                console.log('⚠️ Speed config is empty - using fallback defaults');
              ...
              console.log('🚀 SPEED SCORING without custom config');
              ...
              console.log('📝 STANDARD SCORING');
              // Standard scoring
              points = question.points || 10;
              console.log('✏️ Standard scoring - using question points:', points);
            }
          }
        } else {
          console.log('❌ Answer is INCORRECT - 0 points awarded');
        }
        ...
        console.error('🔍 Error details:', scoringError?.message);
        console.error('📍 Error stack:', scoringError?.stack);
        points = 0; // Fallback to 0 points on error
      }
